...
$(document).on("scroll", function (e) {
  ...
  if (directionScroll[0] < directionScroll[1]) {
    ...
    if (
      ...
      !$(document).find(".header-burger").hasClass("active") &&
      !$(document).find(".header-search").hasClass("active")
    ) {
...
$(document).on("click", ".header-info-wrap, .header-burger", function (e) {
  if (
    !$(".header-info-wrap__tel").is(e.target) &&
    !$(".header-info-wrap-select").is(e.target) &&
    $(".header-info-wrap-select").has(e.target).length === 0
  ) {
...
$(document).on("click touchend", function (e) {
  ...
  if (
    ...
    !div.is(e.target) &&
    div.has(e.target).length === 0 &&
    !$(".header-info-wrap").is(e.target) &&
    $(".header-info-wrap").has(e.target).length === 0 &&
    !$(".header-burger").is(e.target) &&
    $(".header-burger").has(e.target).length === 0
  ) {
    div.removeClass("active");
...
$(document).on("click", ".tabs-navigation-item", function (e) {
  ...
  tabContainers.hide();
  tabContainers.filter(this.dataset.tab).show();
...
$(document).on("click", ".dropDownlists-items-item", function () {
  $(this)
    .toggleClass("active")
    .find(".dropDownlists-items-item__text")
    .toggle();
...
if (horizontalElem) {
  let activeElem = document.querySelector(
      "#horizontalMenu .about-menu__item.active"
    ),
    windowWidth = document.documentElement.clientWidth,
    activeElementWidth = activeElem.offsetWidth;
  horizontalElem.scrollLeft +=
    activeElem.offsetLeft - windowWidth / 2 + activeElementWidth / 2;
}
